...
def solution(p: float, x: np.array) -> tuple:
    ...
    return 2 * loc - 3 * (scale * norm.ppf(1 - alpha / 2) / np.sqrt(len(x))), \
           2 * loc + 3 * (scale * norm.ppf(1 - alpha / 2) / np.sqrt(len(x)))
    #return loc + (scale * norm.ppf(alpha / 2) / np.sqrt(len(x))), \
    #       loc + (scale * norm.ppf(1 - alpha / 2) / np.sqrt(len(x)))
    #return scale
    #return norm.ppf(alpha/2), norm.ppf(1 - alpha/2)
    #return norm.ppf(1 - alpha/2)
    #return loc
    #return loc / (1 - alpha), loc/ alpha

#if __name__ == '__main__':
##    m = np.array([0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 0.96, 0.97, 0.98, 0.99])
#    print(solution(0.9, m))